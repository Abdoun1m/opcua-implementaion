...
if docker exec "$AGENT_CONTAINER" python -m app.main --component-apply-trust --target "$TARGET" --apply-mode trust_only >/tmp/labshock-ot-telemetry-agent-test.log 2>&1; then
  ...
  if rg -q "runtime_write_deprecated_use_native_client|blocked_runtime_write_disabled|target_not_managed_by_agent_zone" /tmp/labshock-ot-telemetry-agent-test.log; then
...
if curl -fsS "$COLLECTOR_SUMMARY_URL" >/tmp/labshock-ot-telemetry-summary.json 2>/tmp/labshock-ot-telemetry-summary.err; then
  if rg -q "security|policy|error|events" /tmp/labshock-ot-telemetry-summary.json; then
...
if docker logs --tail=300 "$AGENT_CONTAINER" >/tmp/labshock-ot-telemetry-agent-logs.txt 2>/tmp/labshock-ot-telemetry-agent-logs.err; then
  if rg -q "ot collector telemetry dropped|runtime_write_deprecated_use_native_client|package_activation_dry_run_staged|certificate_package_rejected|target_not_managed_by_agent_zone" /tmp/labshock-ot-telemetry-agent-logs.txt; then
