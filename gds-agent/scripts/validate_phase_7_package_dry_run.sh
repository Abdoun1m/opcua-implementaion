...
DOCKER_ENVS=(
  -e GDS_CONTROL_PLANE_URL="${GDS_CONTROL_PLANE_URL:-https://192.168.10.30:8443}"
  -e GDS_AGENT_AUTH_ENABLED="${GDS_AGENT_AUTH_ENABLED:-true}"
  -e GDS_AGENT_MTLS_ENABLED="${GDS_AGENT_MTLS_ENABLED:-true}"
  -e GDS_AGENT_TOKEN_FILE="${GDS_AGENT_TOKEN_FILE:-/etc/labshock-gds-agent-auth/token}"
...
for name in \
  ...
  GDS_AGENT_TLS_CERT_FILE \
  GDS_AGENT_TLS_KEY_FILE \
